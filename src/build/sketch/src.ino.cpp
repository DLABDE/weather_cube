#line 1 "e:\\arduino\\code\\code\\weather_cube\\src\\src.ino"
#include "bmp.h"
...
void web_connect_config()
{
    ...
    u8g2.drawStr(0,30,"ID:");u8g2.drawStr(20,42,id);u8g2.sendBuffer();
    u8g2.drawStr(0,52,"PW:");u8g2.drawStr(25,64,pw);u8g2.sendBuffer();
    wifiManager.autoConnect(id,pw);                 //开启wifi连接服务
...
#define FACE_PAGE   3
void (*func_table[FACE_PAGE])() = {time_face,wea_face,sys_face};//指向函数的指针数组
int page=0;

void loop() 
{    
    if (digitalRead(KEY)==HIGH)//按键按下
    {
        state=count;
        if (count-state<=30)
        {
            if (page<FACE_PAGE-1)page++;
            else    page=0;
            u8g2.clear();
        }
    }
...
void get_weather(int a)
{
    digitalWrite(LED_BUILTIN,LOW);Serial.println("get_weather");
